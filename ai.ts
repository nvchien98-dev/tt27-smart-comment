...
export async function generateNLPCBatch({
  students,
  revisionPrompt
}: {
  students: {
  ...
  const prompt = `
Bạn là một giáo viên tiểu học tại Việt Nam, rất am hiểu Thông tư 27/2020/TT-BGDĐT.
Nhiệm vụ của bạn là viết nhận xét Đánh giá Năng lực và Phẩm chất cuối năm cho danh sách học sinh sau đây.

Mỗi học sinh sẽ có danh sách các mức độ đạt được: (T: Tốt, Đ: Đạt, C: Cần cố gắng) cho 3 nhóm:
- Năng lực chung (NLC): Tự chủ và tự học, Giao tiếp và hợp tác, Giải quyết vấn đề và sáng tạo.
- Năng lực đặc thù (NLĐT): Ngôn ngữ, Tính toán, Khoa học, Công nghệ, Tin học, Thẩm mĩ, Thể chất.
- Phẩm chất (PC): Yêu nước, Nhân ái, Chăm chỉ, Trung thực, Trách nhiệm.

Danh sách học sinh:
${students.map(s => `- ID: ${s.id} | Họ tên: ${s.name}
  + NLC: ${s.nlc.join(', ')}
  + NLĐT: ${s.nldt.join(', ')}
  + PC: ${s.pc.join(', ')}`).join('\n')}

Yêu cầu theo Thông tư 27:
- Tổng hợp các mức độ thành 3 câu nhận xét: 1 câu cho NLC, 1 câu cho NLĐT, 1 câu cho PC.
- Khen ngợi nếu nhiều mức T (Tốt).
- Động viên, ghi nhận nếu chủ yếu mức Đ (Đạt).
- Nếu có mức C (Cần cố gắng), nhận xét nhẹ nhàng, định hướng giải pháp.
- KHÔNG nhắc lại tên học sinh trong nhận xét.
${revisionPrompt ? `\nYÊU CẦU BỔ SUNG TỪ GIÁO VIÊN (Rất quan trọng, bắt buộc tuân theo):\n"${revisionPrompt}"` : ''}
...
[
  { 
    "id": "1", 
    "nhanXetNLC": "Tự giác học tập, giao tiếp tốt...", 
    "nhanXetNLDT": "Hoàn thành tốt các môn học...", 
    "nhanXetPC": "Chăm ngoan, đoàn kết với bạn bè..." 
  }
]
`;
  ...
  try {
    ...
    console.error('Error calling Gemini API for NLPC:', error);
    if (error.status === 429) {
       throw new Error('API Rate Limit (Lỗi 429: Quá tải yêu cầu, vui lòng thử lại sau ít phút)');
    }
    throw new Error('Failed to generate NLPC comment: ' + (error.message || 'Unknown error'));
  }
}
